...
public class VkShaderStageFlagBits extends IntEnum<VkShaderStageFlagBits> {
    ...
    */
	...
	/**
     * Creates/populates a array of VkShaderStageFlagBits from a set of values.
     * @param dst - destination array. Can be null.
     * @param values - int[] with valid values of this enumeration.
     * @return VkShaderStageFlagBits array with matching values
     */
    public static VkShaderStageFlagBits[] fromValues(VkShaderStageFlagBits[] dst, int[] values){
        if(values == null) 
               return null;
        if(dst == null || dst.length != values.length){
            dst = new VkShaderStageFlagBits[values.length];
        }        
        for (int i = 0; i < values.length; i++) {
            int val = values[i];
            dst[i] = VkShaderStageFlagBits.fromValue(val);
        }        
        return dst;
    }
