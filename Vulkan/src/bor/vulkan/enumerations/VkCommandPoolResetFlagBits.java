...
public class VkCommandPoolResetFlagBits extends IntEnum<VkCommandPoolResetFlagBits> {
    ...
    */
	...
	/**
     * Creates/populates a array of VkCommandPoolResetFlagBits from a set of values.
     * @param dst - destination array. Can be null.
     * @param values - int[] with valid values of this enumeration.
     * @return VkCommandPoolResetFlagBits array with matching values
     */
    public static VkCommandPoolResetFlagBits[] fromValues(VkCommandPoolResetFlagBits[] dst, int[] values){
        if(values == null) 
               return null;
        if(dst == null || dst.length != values.length){
            dst = new VkCommandPoolResetFlagBits[values.length];
        }        
        for (int i = 0; i < values.length; i++) {
            int val = values[i];
            dst[i] = VkCommandPoolResetFlagBits.fromValue(val);
        }        
        return dst;
    }
