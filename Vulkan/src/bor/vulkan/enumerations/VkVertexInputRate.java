...
public class VkVertexInputRate extends IntEnum<VkVertexInputRate> {
    ...
    */
	...
	/**
     * Creates/populates a array of VkVertexInputRate from a set of values.
     * @param dst - destination array. Can be null.
     * @param values - int[] with valid values of this enumeration.
     * @return VkVertexInputRate array with matching values
     */
    public static VkVertexInputRate[] fromValues(VkVertexInputRate[] dst, int[] values){
        if(values == null) 
               return null;
        if(dst == null || dst.length != values.length){
            dst = new VkVertexInputRate[values.length];
        }        
        for (int i = 0; i < values.length; i++) {
            int val = values[i];
            dst[i] = VkVertexInputRate.fromValue(val);
        }        
        return dst;
    }
