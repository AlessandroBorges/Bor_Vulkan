...
public class VkCommandPoolCreateFlagBits extends IntEnum<VkCommandPoolCreateFlagBits> {
    ...
    */
	...
	/**
     * Creates/populates a array of VkCommandPoolCreateFlagBits from a set of values.
     * @param dst - destination array. Can be null.
     * @param values - int[] with valid values of this enumeration.
     * @return VkCommandPoolCreateFlagBits array with matching values
     */
    public static VkCommandPoolCreateFlagBits[] fromValues(VkCommandPoolCreateFlagBits[] dst, int[] values){
        if(values == null) 
               return null;
        if(dst == null || dst.length != values.length){
            dst = new VkCommandPoolCreateFlagBits[values.length];
        }        
        for (int i = 0; i < values.length; i++) {
            int val = values[i];
            dst[i] = VkCommandPoolCreateFlagBits.fromValue(val);
        }        
        return dst;
    }
