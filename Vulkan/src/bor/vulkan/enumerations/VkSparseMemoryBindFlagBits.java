...
public class VkSparseMemoryBindFlagBits extends IntEnum<VkSparseMemoryBindFlagBits> {
    ...
    */
	...
	/**
     * Creates/populates a array of VkSparseMemoryBindFlagBits from a set of values.
     * @param dst - destination array. Can be null.
     * @param values - int[] with valid values of this enumeration.
     * @return VkSparseMemoryBindFlagBits array with matching values
     */
    public static VkSparseMemoryBindFlagBits[] fromValues(VkSparseMemoryBindFlagBits[] dst, int[] values){
        if(values == null) 
               return null;
        if(dst == null || dst.length != values.length){
            dst = new VkSparseMemoryBindFlagBits[values.length];
        }        
        for (int i = 0; i < values.length; i++) {
            int val = values[i];
            dst[i] = VkSparseMemoryBindFlagBits.fromValue(val);
        }        
        return dst;
    }
