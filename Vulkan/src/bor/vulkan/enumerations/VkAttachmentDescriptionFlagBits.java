...
public class VkAttachmentDescriptionFlagBits extends IntEnum<VkAttachmentDescriptionFlagBits> {
    ...
    */
	...
	/**
     * Creates/populates a array of VkAttachmentDescriptionFlagBits from a set of values.
     * @param dst - destination array. Can be null.
     * @param values - int[] with valid values of this enumeration.
     * @return VkAttachmentDescriptionFlagBits array with matching values
     */
    public static VkAttachmentDescriptionFlagBits[] fromValues(VkAttachmentDescriptionFlagBits[] dst, int[] values){
        if(values == null) 
               return null;
        if(dst == null || dst.length != values.length){
            dst = new VkAttachmentDescriptionFlagBits[values.length];
        }        
        for (int i = 0; i < values.length; i++) {
            int val = values[i];
            dst[i] = VkAttachmentDescriptionFlagBits.fromValue(val);
        }        
        return dst;
    }
