...
public class VkDynamicState extends IntEnum<VkDynamicState> {
    ...
    */
	...
	/**
     * Creates/populates a array of VkDynamicState from a set of values.
     * @param dst - destination array. Can be null.
     * @param values - int[] with valid values of this enumeration.
     * @return VkDynamicState array with matching values
     */
    public static VkDynamicState[] fromValues(VkDynamicState[] dst, int[] values){
        if(values == null) 
               return null;
        if(dst == null || dst.length != values.length){
            dst = new VkDynamicState[values.length];
        }        
        for (int i = 0; i < values.length; i++) {
            int val = values[i];
            dst[i] = VkDynamicState.fromValue(val);
        }        
        return dst;
    }
