...
public class VkLogicOp extends IntEnum<VkLogicOp> {
    ...
    */
	...
	/**
     * Creates/populates a array of VkLogicOp from a set of values.
     * @param dst - destination array. Can be null.
     * @param values - int[] with valid values of this enumeration.
     * @return VkLogicOp array with matching values
     */
    public static VkLogicOp[] fromValues(VkLogicOp[] dst, int[] values){
        if(values == null) 
               return null;
        if(dst == null || dst.length != values.length){
            dst = new VkLogicOp[values.length];
        }        
        for (int i = 0; i < values.length; i++) {
            int val = values[i];
            dst[i] = VkLogicOp.fromValue(val);
        }        
        return dst;
    }
