...
public class VkPhysicalDeviceType extends IntEnum<VkPhysicalDeviceType> {
    ...
    */
	...
	/**
     * Creates/populates a array of VkPhysicalDeviceType from a set of values.
     * @param dst - destination array. Can be null.
     * @param values - int[] with valid values of this enumeration.
     * @return VkPhysicalDeviceType array with matching values
     */
    public static VkPhysicalDeviceType[] fromValues(VkPhysicalDeviceType[] dst, int[] values){
        if(values == null) 
               return null;
        if(dst == null || dst.length != values.length){
            dst = new VkPhysicalDeviceType[values.length];
        }        
        for (int i = 0; i < values.length; i++) {
            int val = values[i];
            dst[i] = VkPhysicalDeviceType.fromValue(val);
        }        
        return dst;
    }
