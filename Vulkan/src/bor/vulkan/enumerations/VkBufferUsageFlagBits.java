...
public class VkBufferUsageFlagBits extends IntEnum<VkBufferUsageFlagBits> {
    ...
    */
	...
	/**
     * Creates/populates a array of VkBufferUsageFlagBits from a set of values.
     * @param dst - destination array. Can be null.
     * @param values - int[] with valid values of this enumeration.
     * @return VkBufferUsageFlagBits array with matching values
     */
    public static VkBufferUsageFlagBits[] fromValues(VkBufferUsageFlagBits[] dst, int[] values){
        if(values == null) 
               return null;
        if(dst == null || dst.length != values.length){
            dst = new VkBufferUsageFlagBits[values.length];
        }        
        for (int i = 0; i < values.length; i++) {
            int val = values[i];
            dst[i] = VkBufferUsageFlagBits.fromValue(val);
        }        
        return dst;
    }
