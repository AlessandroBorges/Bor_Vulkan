...
public class VkAttachmentStoreOp extends IntEnum<VkAttachmentStoreOp> {
    ...
    */
	...
	/**
     * Creates/populates a array of VkAttachmentStoreOp from a set of values.
     * @param dst - destination array. Can be null.
     * @param values - int[] with valid values of this enumeration.
     * @return VkAttachmentStoreOp array with matching values
     */
    public static VkAttachmentStoreOp[] fromValues(VkAttachmentStoreOp[] dst, int[] values){
        if(values == null) 
               return null;
        if(dst == null || dst.length != values.length){
            dst = new VkAttachmentStoreOp[values.length];
        }        
        for (int i = 0; i < values.length; i++) {
            int val = values[i];
            dst[i] = VkAttachmentStoreOp.fromValue(val);
        }        
        return dst;
    }
